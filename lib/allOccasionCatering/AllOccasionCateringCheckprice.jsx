...
const AllOccasionCateringCheckprice = () => {
    ...
    const [aocVegGravy, setAocVegGravy] = React.useState([]);
    const [aocVegStarter, setAocVegStarter] = React.useState([]);
    const [aocVegDry, setAocVegDry] = React.useState([]);
    const [aocDal, setAocDal] = React.useState([]);
    const [aocRice, setAocRice] = React.useState([]);
    const [aocSalad, setAocSalad] = React.useState([]);
    const [aocBread, setAocBread] = React.useState([]);
    const [aocKofta, setAocKofta] = React.useState([]);
    const [aocDessert, setAocDessert] = React.useState([]);
    const [aocChickenStarter, setAocChickenStarter] = React.useState([]);
    const [aocFishStarter, setAocFishStarter] = React.useState([]);
    const [aocChickenGravy, setAocChickenGravy] = React.useState([]);
    const [aocFishGravy, setAocFishGravy] = React.useState([]);
    ...
    if ((vegCount == "" || vegCount == 0) && (nvCount != "" || nvCount != 0)) {
        vegCount = nvCount;
        vegSnackPrice = (aocVegGravy.reduce((sum, item) => sum + item.price, 0) + aocVegStarter.reduce((sum, item) => sum + item.price, 0) + aocVegDry.reduce((sum, item) => sum + item.price, 0) + aocVegDry.reduce((sum, item) => sum + item.price, 0)) * vegCount;
    } else {
        vegSnackPrice = (aocVegGravy.reduce((sum, item) => sum + item.price, 0) + aocVegStarter.reduce((sum, item) => sum + item.price, 0) + aocVegDry.reduce((sum, item) => sum + item.price, 0) + aocVegDry.reduce((sum, item) => sum + item.price, 0)) * vegCount;
    }
    ...
    const nvSnackPrice = (aocChickenStarter.reduce((sum, item) => sum + item.price, 0) + aocFishStarter.reduce((sum, item) => sum + item.price, 0) + aocChickenGravy.reduce((sum, item) => sum + item.price, 0) + aocFishGravy.reduce((sum, item) => sum + item.price, 0)) * nvCount;
    console.log(nvSnackPrice)
    // Calculate the dessert price based on the total number of people
    const dessertPrice = (aocDal.reduce((sum, item) => sum + item.price, 0) + aocRice.reduce((sum, item) => sum + item.price, 0) + aocSalad.reduce((sum, item) => sum + item.price, 0) + aocBread.reduce((sum, item) => sum + item.price, 0) + aocKofta.reduce((sum, item) => sum + item.price, 0) + aocDessert.reduce((sum, item) => sum + item.price, 0)) * totalGuestCount;
    ...
    React.useEffect(() => {
        const savedCheckedValues = sessionStorage.getItem('aocVegGravy');
        const savedCheckedValues2 = sessionStorage.getItem('aocVegStarter');
        const savedCheckedValues3 = sessionStorage.getItem('aocVegDry');
        const savedCheckedValues4 = sessionStorage.getItem('aocDal');
        const savedCheckedValues5 = sessionStorage.getItem('aocRice');
        const savedCheckedValues6 = sessionStorage.getItem('aocSalad');
        const savedCheckedValues7 = sessionStorage.getItem('aocBread');
        const savedCheckedValues8 = sessionStorage.getItem('aocKofta');
        const savedCheckedValues9 = sessionStorage.getItem('aocDessert');
        const savedCheckedValues10 = sessionStorage.getItem('aocChickenStarter');
        const savedCheckedValues11 = sessionStorage.getItem('aocFishStarter');
        const savedCheckedValues12 = sessionStorage.getItem('aocChickenGravy');
        const savedCheckedValues13 = sessionStorage.getItem('aocFishGravy');

        if (savedCheckedValues) {
            setAocVegGravy(JSON.parse(savedCheckedValues));
        }
        if (savedCheckedValues2) {
            setAocVegStarter(JSON.parse(savedCheckedValues2));
        }
        if (savedCheckedValues3) {
            setAocVegDry(JSON.parse(savedCheckedValues3));
        }
        if (savedCheckedValues4) {
            setAocDal(JSON.parse(savedCheckedValues4));
        }
        if (savedCheckedValues5) {
            setAocRice(JSON.parse(savedCheckedValues5));
        }
        if (savedCheckedValues6) {
            setAocSalad(JSON.parse(savedCheckedValues6));
        }
        if (savedCheckedValues7) {
            setAocBread(JSON.parse(savedCheckedValues7));
        }
        if (savedCheckedValues8) {
            setAocKofta(JSON.parse(savedCheckedValues8));
        }
        if (savedCheckedValues9) {
            setAocDessert(JSON.parse(savedCheckedValues9));
        }
        if (savedCheckedValues10) {
            setAocChickenStarter(JSON.parse(savedCheckedValues10));
        }
        if (savedCheckedValues11) {
            setAocFishStarter(JSON.parse(savedCheckedValues11));
        }
        if (savedCheckedValues12) {
            setAocChickenGravy(JSON.parse(savedCheckedValues12));
        }
        if (savedCheckedValues13) {
            setAocFishGravy(JSON.parse(savedCheckedValues13));
        }
    }, []);
    ...
    return (
        <div style={backgroundStyle} className={styles.mainBody}>
            ...
            <div className={styles.selectedItems}>
                <h5>Veg Starter</h5>
                {aocVegStarter.length > 0 ? (aocVegStarter.map((vegSnackdata, index) => (<div key={index} className='d-flex'>
                    <div>
            ...
            { nvCount > 0 ? <div className={styles.selectedItems}>
                <h5>Chicken Starter</h5>
                {aocChickenStarter.length > 0 ? (aocChickenStarter.map((vegHeavySnackdata, index) => (<div key={index} className='d-flex'>
                    <div>
            ...
            {nvCount > 0 ?<div className={styles.selectedItems}>
                <h5>Fish Starter</h5>
                {aocFishStarter.length > 0 ? (aocFishStarter.map((vegHeavySnackdata, index) => (<div key={index} className='d-flex'>
                    <div>
                        <Image src="/birthdayParty/nvlogo.png" width="11.852px" height="11.852px" />
                    </div>
                    <div>
            ...
            <div className={styles.selectedItems}>
                <h5>Salad</h5>
                {aocSalad.length > 0 ? (aocSalad.map((nonVegSnackData, index) => (<div key={index} className='d-flex'>
                    <div>
                        <Image src="/birthdayParty/vegLogo.png" width="11.852px" height="11.852px" />
                    </div>
                    <div>
            ...
            <div className={styles.selectedItems}>
                <h5>Bread</h5>
                {aocBread.length > 0 ? (aocBread.map((nonVegHeavySnackData, index) => (<div key={index} className='d-flex'>
                    <div>
                        <Image src="/birthdayParty/vegLogo.png" width="11.852px" height="11.852px" />
                    </div>
                    <div>
            ...
            <div className={styles.selectedItems}>
                <h5>Rice</h5>
                {aocRice.length > 0 ? (aocRice.map((nonVegHeavySnackData, index) => (<div key={index} className='d-flex'>
                    <div>
                        <Image src="/birthdayParty/vegLogo.png" width="11.852px" height="11.852px" />
                    </div>
                    <div>
            ...
            <div className={styles.selectedItems}>
                <h5>Veg Dry</h5>
                {aocVegDry.length > 0 ? (aocVegDry.map((dessertData, index) => (<div key={index} className='d-flex'>
                    <div>
                        <Image src="/birthdayParty/vegLogo.png" width="11.852px" height="11.852px" />
                    </div>
                    <div>
            ...
            <div className={styles.selectedItems}>
                <h5>Veg Gravy</h5>
                {aocVegGravy.length > 0 ? (aocVegGravy.map((dessertData, index) => (<div key={index} className='d-flex'>
                    <div>
                        <Image src="/birthdayParty/vegLogo.png" width="11.852px" height="11.852px" />
                    </div>
                    <div>
            ...
            <div className={styles.selectedItems}>
                <h5>Dal</h5>
                {aocDal.length > 0 ? (aocDal.map((dessertData, index) => (<div key={index} className='d-flex'>
                    <div>
                        <Image src="/birthdayParty/vegLogo.png" width="11.852px" height="11.852px" />
                    </div>
                    <div>
            ...
            <div className={styles.selectedItems}>
                <h5>Kofta</h5>
                {aocKofta.length > 0 ? (aocKofta.map((dessertData, index) => (<div key={index} className='d-flex'>
                    <div>
                        <Image src="/birthdayParty/vegLogo.png" width="11.852px" height="11.852px" />
                    </div>
                    <div>
            ...
            <div className={styles.selectedItems}>
                <h5>Dessert</h5>
                {aocDessert.length > 0 ? (aocDessert.map((dessertData, index) => (<div key={index} className='d-flex'>
                    <div>
                        <Image src="/birthdayParty/vegLogo.png" width="11.852px" height="11.852px" />
                    </div>
                    <div>
            ...
            { nvCount > 0 ? <div className={styles.selectedItems}>
                <h5>Chicken Gravy</h5>
                {aocChickenGravy.length > 0 ? (aocChickenGravy.map((dessertData, index) => (<div key={index} className='d-flex'>
                    <div>
                        <Image src="/birthdayParty/nvlogo.png" width="11.852px" height="11.852px" />
                    </div>
                    <div>
            ...
            {nvCount > 0 ?<div className={styles.selectedItems}>
                <h5>Fish Gravy</h5>
                {aocFishGravy.length > 0 ? (aocFishGravy.map((dessertData, index) => (<div key={index} className='d-flex'>
                    <div>
                        <Image src="/birthdayParty/nvlogo.png" width="11.852px" height="11.852px" />
                    </div>
                    <div>
            ...
            <div className={styles.orderbuttonsectn}>
                <div className={styles.bookinghelpbtn} style={{ margin: "auto" }}>
                    <Link href="https://api.whatsapp.com/send?phone=917738096313&text=Hey!%20Need%20help%20booking%20a%20AllOccasionCatering"><button>Get Booking Help</button></Link>
                </div>
...
export default AllOccasionCateringCheckprice
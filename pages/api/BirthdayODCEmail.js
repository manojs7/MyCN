  ...
  if (req.method === "POST") {
    ...
    try {
      let mailOptions = {
        ...
        to: `${data.email}, caterninjadiy@gmail.com, ${
          data.emailedtoparser ? "" : "i4e16n207_bvnzocb@parser.zohocrm.com"
        }`,
    ...
    <h3>Veg Snack :
          ...
          <br>
          <h3>Non Veg Heavy Snack :
          ${
            // data.appetizer.some((el) => el.value === true) &&
            data.nonVegHeavySnack
              ? data.nonVegHeavySnack
                  .map((array, index) => {
                    return array.name
                      ? `<li key="${array.id}_${index}">${array.name} </li>`
      ...
      <h3>Box/Buffet Price : ${
        data.buffet ? "NinjaBuffet" : "NinjaBox"
      }</h3><br>
